...
def main():
	...
	make_blast_db = ["makeblastdb", "-in", reference, "-dbtype",  "nucl", "-out", "tmp.aai.reference.db"]
	subprocess.call(make_blast_db)
	blast = ["tblastn",  "-query", query, "-db", "tmp.aai.reference.db", "-out", "tmp.aai.blast.out", "-outfmt", '6 qseqid qlen qstart qend sseqid slen sstart send length nident mismatch gaps score bitscore evalue pident']
	subprocess.call(blast)

	blast_dataframe = pd.read_table("tmp.aai.blast.out", sep="\t", header=None)
	...
	for i in range(len(blast_dataframe)):
		...
		if (amino_percent_match > 0.70 and blast_dataframe.iloc[i::+1, 15].values[0] > 0.30):
			sum_amino_percent_match += amino_percent_match
			qseq_length += blast_dataframe.iloc[i::+1, 9].values[0]
			rseq_length += blast_dataframe.iloc[i::+1, 8].values[0]
			number_of_fragments += 1

	score = ((sum_amino_percent_match // number_of_fragments ) ) * 100
	coverage = (qseq_length // rseq_length ) * 100
	similarity = coverage * score // 100

	print
	print sum_amino_percent_match, "\t", number_of_fragments, "\t", qseq_length, "\t", rseq_length, "\t", coverage, "\t", score, "\t", similarity
	print
