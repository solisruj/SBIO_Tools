...
def create_parser():
	parser = argparse.ArgumentParser(description="This program returns Average Nucleotide Identity Coverage, Score, and Similarity for mutltiple references.")
	parser.add_argument("-q", dest="query", type=str, nargs=1 , help="Input nucleotide sequence file (.fna/.fasta).", required=True)
	parser.add_argument("-r", dest="reference", type=str, nargs=1, help="Reference directory containing reference files (.fna/.fasta).", required=True) 
...
def main():
	...
	for reference in os.listdir(ref):
		if(reference.endswith('.fna') or reference.endswith('.fasta')):
			
			header.append(reference)
			ani_cmnd = ['Average_Nucleotide_Identity.py', '-q', query, '-r', ref + '/' + reference]
